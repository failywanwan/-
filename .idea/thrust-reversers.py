...
def aripartspage():
    ...
    #获得页面页数
    for i in range(1,3):
        baseurl="https://www.godirecttrade.com/thrust-reversers.html"
        pageurl="?p="+str(i)
        url=baseurl+pageurl
        driver.get(url)
...
def export_excel(dic_data):
    ...
    # 指定生成的Excel表格名称
    file_path=pd.ExcelWriter('thrust-reversers.xlsx')
...
def test():
    airparts=[]
    url="https://www.godirecttrade.com/has-group-pn-an960-716l-ne.html"
    driver.get(url)
    #1
    try:
        sku=driver.find_element_by_xpath('//*[@id="maincontent"]/div[2]/div/div[1]/div/div/div[1]/div[1]/div/div[2]/div/div[2]/div/div/div[1]/div[2]/h1').text
